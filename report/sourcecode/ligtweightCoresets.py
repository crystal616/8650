...
Construct corsets using LWCS algorithm
...
ref: Olivier Bachem, Mario Lucic, Andreas Krause. "scalable k-means clustering via lightweight corests". KDD2018.
...
if __name__ == '__main__':
    ...
    parser.add_argument('--numOfCore', type=int, default=1, help='number of cores to use')
    ...
    for sampleTimes in range(numOfCorset):
        chosen=np.random.choice(size, samplesize, replace=False, p=posibilities)
        with open(filename[:-4]+'_LWCS_'+str(samplesize)+"_"+str(sampleTimes+1) + ".txt",'w',encoding='utf-8') as f:
            for item in chosen:
                f.write(str(1/(samplesize*float(posibilities[item])))+' ')
